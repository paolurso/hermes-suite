...
```bash
podman pull ascensionoid/hermes-suite:2026.6.5-0.51.386
```
...
podman build \
  ...
  --build-arg HERMES_WEBUI_VERSION=v0.51.386 \
  -t hermes-suite:2026.6.5-0.51.386 .
```
...
# Override defaults:
# ./build.sh --agent v2026.6.5 --webui v0.51.386
```
...
|-----------|---------------|---------------|--------|
| `2026.6.5-0.51.386` | v2026.6.5 | v0.51.386 | amd64 + arm64 |
...
- **WebUI**: semantic version from `nesquena/hermes-webui` (e.g. `v0.51.386`)
...
podman build \
  --build-arg AGENT_VERSION=v2026.6.5 \
  --build-arg HERMES_WEBUI_VERSION=v0.51.386 \
  -t ascensionoid/hermes-suite:2026.6.5-0.51.386 .
```
